...
import { ignoredDirs } from "../../utils/scanFilters.js"
...
export async function scanDirectory(dir: string, searchValue: string): Promise<boolean> {
  try {
    ...
    const filtered = entries.filter(entry => !ignoredDirs.includes(entry.name)) // Exclude ignored directories
    ...
    for (const entry of filtered) {
      ...
      if (entry.isDirectory()) {
        ...
        await scanDirectory(fullPath, searchValue)
      } else if (entry.isFile()) {
        // If the entry is a file, analyze its content for matches
        await analyzeFile(fullPath, searchValue)
      }
    }
 ...
 */
async function analyzeFile(filePath: string, searchValue: string) {
